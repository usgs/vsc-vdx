...
/**
 * <p>
 * <code>Wave</code> is a class that handles regularly sampled time-series data
 * like a seismic waveform. This class is well suited to higher sampling rate
 * data (>10Hz) with little or no gaps.
 * <p>
...
public class Wave implements BinaryDataSet, Comparable<Wave>, Cloneable {
	protected final static Logger logger = Log
			.getLogger("gov.usgs.vdx.data.wave.Wave");
	...
	/**
	 * Duplicates another <code>Wave</code> performing a deep copy of the
	 * samples array.
	 * 
	 * @param wave
	 *            the source wave
	...
	/**
	 * Constructs a <code>Wave</code> from an array of bytes. This constructor
	 * is best used when transmitting <code>Waves</code> over a network.
	 * <p>
	...
	/**
	 * Computes the FFT of the <code>Wave</code>. This function will perform the
	 * necessary zero-padding in order to make the samples buffer be a power of
	 * 2 in size.
	 * 
	...
	/**
	 * Invalidates the cached statistics. TODO: document when and why this
	 * should be called
	 */
	...
	public List<Wave> split(int maxSamples) {
		ArrayList<Wave> list = new ArrayList<Wave>(buffer.length / maxSamples
				+ 2);
		double ct = startTime;
		int j = 0;
	...
	/**
	 * Expiremental. For use with Winston static importers. TODO: handle erase a
	 * chunk inside the existing wave
	 * 
	 * @param t1
	 *            start time
	 * @param t2
...
//		if (rightWave.adjacent(leftWave))
//			i++;
	...
	public static Wave join(List<Wave> waves) {
		...
		for (Wave sw : waves) {
			...
			// System.out.println("join: " + i + " " + buffer.length + " " +
			// sw.buffer.length);
	...
	/**
	 * Joins together a list of waves into one large wave. Forces extent of wave
	 * from arguments so sort order does not matter.
	 * 
	 * @param waves
	 *            the list of <code>Wave</code> s
	 * @param t1
	...
	public static Wave join(List<Wave> waves, double t1, double t2) {
		...
		for (Wave sw : waves) {
			int i = (int) Math.round((sw.getStartTime() - t1)
					* wv0.getSamplingRate());
			System.arraycopy(sw.buffer, 0, buffer, i, sw.buffer.length);
		}
	...
	/**
	 * Gets an estimate for the size in RAM of this <code>Wave</code>. It
	...
	/**
	 * Exports this <code>Wave</code> to a simple text file. This function
	 * assumes that the time is in J2K seconds and converts it into Unix
	 * milliseconds. The output is a simple 2-D rectangular text file, first
	 * column time, second column sample.
	 * 
	 * @param fn
	 *            the output filename
	 */
	 ...
	 */
	/*
	 * public SAC toSAC() { SAC sac = new SAC(); sac.npts = buffer.length;
	 * Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("GMT"));
	 * cal.setTime(Util.j2KToDate(startTime)); sac.nzyear =
	 * cal.get(Calendar.YEAR); sac.nzjday = cal.get(Calendar.DAY_OF_YEAR);
	 * sac.nzhour = cal.get(Calendar.HOUR_OF_DAY); sac.nzmin =
	 * cal.get(Calendar.MINUTE); sac.nzsec = cal.get(Calendar.SECOND);
	 * sac.nzmsec = cal.get(Calendar.MILLISECOND); sac.delta = (float)(1 /
	 * samplingRate); sac.y = new float[buffer.length]; for (int i = 0; i <
	 * buffer.length; i++) sac.y[i] = buffer[i]; return sac; }
	 */
	 ...
	 * 
	 * TODO: throw exception TODO: get rid of IntVector
	 * 
	 * @param fn
	 *            the import filename
	 * @return the wave
	...
	public static Wave importFromText(String fn) {
		try {
			...
			sw.startTime = Double.parseDouble(firstLine.substring(0,
					firstLine.indexOf(" ")));
			iv.add(Integer.parseInt(firstLine.substring(firstLine.indexOf(" ") + 1)));
			String secondLine = in.readLine();
			double nt = Double.parseDouble(secondLine.substring(0,
					secondLine.indexOf(" ")));
	 ...
	 * <code>Wave</code>. This is used before sending a <code>Wave</code> over a
	 * network (or potentially into a file). The <code>ByteBuffer</code> needs
	 * to be flipped before it can fed somewhere.
	 * 
	...
	public String toString() {
		return "Wave: startTime=" + startTime + ", endTime=" + getEndTime()
				+ ", samplingRate=" + samplingRate + ", samples="
				+ buffer.length + "\nstartDate="
				+ Util.j2KToDateString(startTime) + "\nendDate="
				+ Util.j2KToDateString(getEndTime())
				+ ((dataType == null) ? "" : ("\ndataType=" + dataType));
	}
	 ...
	 * 
	 * @param p
	 *            period used for rolling median
	 */
	 ...
	 * 
	 * @param p
	 *            period for rolling mean
	 * 
	 *            public void despike_mean(double p ) { double sum =
	 *            (double)buffer[0]; double x0 = 0; int i = 1; int r =
	 *            this.buffer.length;
	 * 
	 *            // Initialize window for ( i=1; i<r; i++ ) { if (
	 *            (i-x0)*samplingRate > p ) break; sum += buffer[i]; } int count
	 *            = i; int j = 0;
	 * 
	 *            do { // Remove 1st element in the window & despike it
	 *            buffer[j] -= (sum/count); j++; x0 = buffer[j]; count--; //
	 *            Refill window for ( ; i<r; i++ ) { if ( (i-x0)*samplingRate >
	 *            p ) break; sum += (double)buffer[i]; count++; } } while ( i<r
	 *            );
	 * 
	 *            // Despike elements left in window double mean = (sum/count);
	 *            for ( ; j<r; j++ ) buffer[j] -= mean; }
	 */
}