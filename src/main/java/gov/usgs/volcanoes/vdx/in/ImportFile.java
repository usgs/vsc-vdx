...
public class ImportFile extends Import implements Importer {
  ...
  public void processConfigFile(String configFile) {
    ...
    dateIn = new SimpleDateFormat(
        StringUtils.stringToString(params.getString("timestamp"), "yyyy-MM-dd HH:mm:ss"));
    dateIn.setTimeZone(
    ...
    if (stringList != null) {
      for (int i = 0; i < stringList.size(); i++) {
        ...
        channelFieldMap.put(channelCode,
            StringUtils.stringToString(channelParams.getString("fields"), fields));
      }
    }
    ...
    for (int i = 0; i < dataSourceList.size(); i++) {
      ...
      if (sqlDataSource.getColumnsFlag()) {
        ...
        if (stringList != null) {
          for (int j = 0; j < stringList.size(); j++) {
            ...
            columnDescription = StringUtils
                .stringToString(columnParams.getString("description"), columnName);
            ...
            columnAccumulate = StringUtils
                .stringToBoolean(columnParams.getString("accumulate"), false);
      ...
      if (sqlDataSource.getChannelsFlag() && channels.length() > 0) {
        ...
        for (int j = 0; j < channelArray.length; j++) {
          ...
          if (sqlDataSource
              .defaultGetChannel(channel.getCode(), sqlDataSource.getChannelTypesFlag()) == null) {
            ...
            channel = sqlDataSource
                .defaultGetChannel(channel.getCode(), sqlDataSource.getChannelTypesFlag());
            channelMap.put(channelCode, channel);
          }

          // create a new translation if any non-default values were specified,
          // and use the new tid for the create channel statement
          if (sqlDataSource.getTranslationsFlag()) {
            int extraColumn = 0;
            double multiplier;
            double offset;
            ...
            for (int k = 0; k < columnList.size(); k++) {
              ...
              multiplier = StringUtils
                  .stringToDouble(translationParams.getString("c" + columnName), 1);
  ...
  public void process(String filename) {
    ...
    try {
      ...
      // reset the channel code, as it will be derived from the filename,
      // and not the config file, or the contents of the file
      channelCode = "";
      ...
      while (line != null) {
        ...
        // split the data row into an ordered list. be sure to use the two argument split,
        // as some lines may have many trailing delimiters
        ...
        // log the line to the log file that is being imported,
        // now that all potential errors have been caught
        ...
        for (int i = 0; i < dataSourceList.size(); i++) {
          ...
          if (sqlDataSource.getChannelsFlag()) {
            if (sqlDataSource.defaultGetChannel(channelCode, sqlDataSource.getChannelTypesFlag())
                == null) {
              sqlDataSource.defaultCreateChannel(
                  new Channel(0, channelCode, channelCode, Double.NaN, Double.NaN, Double.NaN, 1),
                  1,
                  sqlDataSource.getChannelsFlag(), sqlDataSource.getTranslationsFlag(),
                  sqlDataSource.getRanksFlag(), sqlDataSource.getColumnsFlag());
            }
