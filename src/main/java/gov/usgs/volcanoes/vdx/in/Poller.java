...
public abstract class Poller extends Thread {
  ...
  /**
   * the number of milliseconds until the next poll.  If you want to start polling at a certain time
   * then do regular interval polling after that time you can use this variable
   */
  ...
  /**
   * Creates a new stopped, alive Poller and starts the Thread.  This does NOT start the polling it
   * just gets the Thread started; you must call startPolling to start the polling.
   */
  public Poller() {
    super();
    stopped = true;
  ...
  /**
   * The Thread's run() method.  This takes care of the polling timing and the calling of the poll()
   * method.
   */
  public void run() {
    while (true) {
      if (!stopped) {
        try {
          ...
          LOGGER.error("Poller sleep interrupted.", ex);
        }
