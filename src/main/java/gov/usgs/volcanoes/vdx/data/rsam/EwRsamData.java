...
/**
 * A class that deals with precomputed RSAM data.  The data are stored in a 2-D matrix, the first
 * column is the time (j2ksec), the second is the data.
 *
 * @author Tom Parker
 * @author Bill Tollett
 */
public class EwRsamData extends RSAMData {
  ...
  protected static final Logger LOGGER = LoggerFactory.getLogger(EwRsamData.class);
  ...
  /**
   * Dump RSAMData to byte buffer. This first 4 bytes specify an integer number of rows followed by
   * rows*16 bytes, 2 doubles: j2ksec and RSAM.
   *
