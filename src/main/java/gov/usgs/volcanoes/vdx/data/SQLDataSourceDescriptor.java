...
public class SQLDataSourceDescriptor {
  ...
  /**
   * Getter for data source class name return class name.
   */
  ...
  private void instantiateDataSource() {
    try {
      ...
      Class.forName(className).getMethod("initialize", new Class[] {ConfigFile.class})
          .invoke(sqlDataSource, new Object[] {params});
    } catch (Exception ex) {
      ex.printStackTrace();
    }
  }
