...
public class SQLGpsDataSource extends SQLDataSource implements DataSource {
  ...
  public boolean createChannel(String channelCode, String channelName, double lon, double lat,
    ...
    return defaultCreateChannel(channelCode, channelName, lon, lat, height, active, 0, channels,
   ...
   * @return GpsData
   */
  public GpsData getGpsData(int cid, int rid, double st, double et, int maxrows,
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (Double.compare(tempJ2ksec, rs.getDouble(1)) != 0) {
          dp = new DataPoint();
          dp.timeVal = getDoubleNullCheck(rs, 1);
          dp.rankVal = getDoubleNullCheck(rs, 2);
          dp.xcoord = getDoubleNullCheck(rs, 3);
          dp.ycoord = getDoubleNullCheck(rs, 4);
          dp.zcoord = getDoubleNullCheck(rs, 5);
  ...
  /**
   * Insert a source file entry to the database. If the filename/rank combination already exist then
   * they are first deleted to allow for an overwrite
   *
  ...
  public int insertSource(String name, String hash, double t0, double t1, int rid) {
    ...
    try {
      ...
      ps = database.getPreparedStatement(
          "SELECT sid FROM sources WHERE j2ksec0 = ? AND j2ksec1 = ? AND rid = ?");
      ps.setDouble(1, t0);
      ps.setDouble(2, t1);
      ps.setInt(3, rid);
      rs = ps.executeQuery();
      ...
      ps = database.getPreparedStatement(
          "INSERT INTO sources (name, hash, j2ksec0, j2ksec1, rid) VALUES (?,?,?,?,?)");
      ps.setString(1, name);
      ps.setString(2, hash);
      ps.setDouble(3, t0);
      ps.setDouble(4, t1);
  ...
  public int insertSourceSimple(String name, double t0, double t1, int rid) {
    ...
    try {
      ...
      if (rs.next()) {
        ...
        Rank rank;
        rank = defaultGetRank(rid);
        LOGGER.error("deleted j2ksec0:{} / j2ksec1:{} / rank:{} (solutions)", String.valueOf(t0),
            String.valueOf(t1), rank.getName(), delcount);
      }

      // it is now safe to insert this NEW source
      ps = database.getPreparedStatement(
          "INSERT INTO sources (name, j2ksec0, j2ksec1, rid) VALUES (?,?,?,?)");
      ps.setString(1, name);
      ps.setDouble(2, t0);
      ps.setDouble(3, t1);
      ps.setInt(4, rid);
  ...
  public void insertSolution(int sid, int cid, DataPoint dp) {
    ...
    try {
      ...
      ps.setInt(1, sid);
      ps.setInt(2, cid);
      ps.setDouble(3, dp.xcoord);
      ps.setDouble(4, dp.ycoord);
      ps.setDouble(5, dp.zcoord);
