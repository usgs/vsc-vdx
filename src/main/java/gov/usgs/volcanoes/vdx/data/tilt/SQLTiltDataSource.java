...
public class SQLTiltDataSource extends SQLDataSource implements DataSource {
  ...
  public boolean createDatabase() {
    ...
    try {
      ...
      // alter the translations table to add an azimuth column
  ...
  public boolean createChannel(String channelCode, String channelName, double lon, double lat,
      double height, int active, int tid, double azimuth) {
  ...
  public RequestResult getTiltData(int cid, int rid, double st, double et, int maxrows,
    ...
    try {
      ...
      sql = "SELECT j2ksec, c.rid, "
          ...
          + "   -SIN(RADIANS(b.azimuth)) * (xTilt * cxTilt + dxTilt) "
          + "       + COS(RADIANS(b.azimuth)) * (yTilt * cyTilt + dyTilt), "
          + "   holeTemp * choleTemp + dholeTemp, "
          + "   boxTemp  * cboxTemp  + dboxTemp,  "
          + "   instVolt * cinstVolt + dinstVolt, "
          + "   rainfall * crainfall + drainfall  "
  ...
  public List<String> getAzimuths() {
    ...
    try {
      database.useDatabase(dbName);
      rs = database.getPreparedStatement("SELECT cid, azimuth FROM channels ORDER BY cid")
          .executeQuery();
      while (rs.next()) {
