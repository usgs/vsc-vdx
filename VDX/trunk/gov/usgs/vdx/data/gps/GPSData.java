...
/**
 * A class that deals with GPS data.  The data are stored in three matrices:
 * an Nx1 time matrix, a (3*N)x1 position matrix, and a sparse (3*N)x(3*N) 
 * covariance matrix, where N is the number of observations.
 *
...
public class GPSData implements BinaryDataSet
{
	...
	public ByteBuffer toBinary()
	{
		...
		ByteBuffer bb = ByteBuffer.allocate(4 + rows * 11 * 8);
		bb.putInt(rows);
		for (int i = 0; i < rows; i++)
		{
	 ...
	 */
	public void setData(DoubleMatrix2D t, DoubleMatrix2D xyz, DoubleMatrix2D cov)
	{
		tData = t;
		xyzData = xyz;
		covData = cov;
...
//		output();
	...
	/** Generates position/time time-series data for plotting by Valve.  The
	 * returned time-series data is simply an array of j2ksec, e, n, u, length
	 * arrays.
	 * @param baseline baseline data or null for none
	 * @return time-series data
	 */
	public double[][] toTimeSeries(GPSData baseline)
	{
		...
		DoubleMatrix2D bigMatrix = DoubleFactory2D.dense.compose(new DoubleMatrix2D[][] {{tData, enuRows, lenData}});
