...
public class Spectrogram {
	...
	public Spectrogram(double[] s) {
		...
		this(s, DEFAULT_SAMPLING_RATE, DEFAULT_NFFT, DEFAULT_BIN_SIZE,
				DEFAULT_OVERLAP, DEFAULT_BETA);
	...
	public Spectrogram(double[] s, int sr, int nf, int bs, int ol, double b) {
		...
		nRows = nfft / 2 + 1;
		nColumns = (int) Math.floor(((double) getNSamples() - (double) overlap)
				/ ((double) binSize - (double) overlap));
	...
	public double[][] getLogSpectraAmplitude(double multiplier,
		...
		for (int i = 0; i < nRows; i++)
			for (int j = 0; j < nColumns; j++)
				logAmp[i][j] = multiplier
						* Math.log10(spectraAmplitude[i][j]
								/ reference_amplitude);
	...
	private double[] computeTime() {
		...
		double delta = ((double) binSize - (double) overlap)
				/ (double) samplingRate;
	...
	private double[][] computeSpectraAmplitude() {
		...
		for (int i = 0; i < nColumns; i++) {
			...
			for (int j = 2; j < nfft; j = j + 2) {
				...
				// is 117
				specAmp[j / 2][i] = Math.sqrt(bin[j] * bin[j] + bin[j + 1]
						* bin[j + 1]);
				bin[j] = 0;
				bin[j + 1] = 0;
			}
