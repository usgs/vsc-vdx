...
public class ImportMetadata {
	...
	public void process(String pathname) {
		...
		int defaultRank = 0;
		int lineLen1, lineLen2; // min & max # of args on each input line
		...
		while (line != null) {
			...
			try {
				...
				logger.warning("Aborting import of line " + lineNumber + ", unknown channel: " + valueArray[1]);
				continue;
			}
			if ( ds != null ) {
				try {
					...
					logger.warning("Aborting import of line " + lineNumber + ", unknown column: " + valueArray[4]);
					continue;
				}
				if ( valueArrayLength == lineLen1 ) {
					...
					try {
						...
						logger.warning("Aborting import of line " + lineNumber + ", unknown rank: " + valueArray[5]);
						continue;
					}
				}
			} else {
