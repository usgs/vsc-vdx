...
public class ImportStacov {	
	...
	public ImportStacov(String ds, int rank) {
		...
		database = VDXDatabase.getVDXDatabase("VDX.config");
		if (database == null) {
			logger.severe("Could not connect to VDX database");
			System.exit(-1);
		} else {
			logger.info("Connected to VDX database ...");
		}
		...
		dataSource.initialize(database, name);
