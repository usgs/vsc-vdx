...
public class TensorstrainData extends GenericDataMatrix {
	 ...
	 * 
	 * @param pts
	 *            list of raws, each of them is double[3]
	 */
	...
	public DoubleMatrix2D getRotatedDataWithoutTime(double theta) {
		...
		for(int i = 0; i<data.rows(); i++){
			en.setQuick(i, 0, data.getQuick(i, 7)*Math.cos(2*tr) + data.getQuick(i, 8)*Math.sin(2*tr));
			en.setQuick(i, 1, -data.getQuick(i, 7)*Math.sin(2*tr) + data.getQuick(i, 8)*Math.cos(2*tr));
		}
	    return en;
	}
}
