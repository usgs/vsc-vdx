...
public class SliceWaveRenderer extends FrameRenderer {
    ...
    public Pair<Double, Double> getYLimits() {
    ...
    public void createDefaultLegendRenderer(String[] s) {
        ...
        for (int i = 0; i < s.length; i++)
            if (s[i] != null) getLegendRenderer().addLine(sr, null, s[i]);
    }
    ...
    protected class DefaultWaveFrameDecorator extends DefaultFrameDecorator {
        public DefaultWaveFrameDecorator() {
            ...
            if (xUnits) {
                this.xUnit =
                        timeZone + " Time (" + Util.j2KToDateString(viewStartTime, dateFormatString) + " to " +
                                Util.j2KToDateString(viewEndTime, dateFormatString) + ")";
            }
            if (yLabelText != null) {
     ...
     */
    public void update() {
        if (decorator == null) createDefaultFrameDecorator();
        if (decorator instanceof DefaultFrameDecorator) ((DefaultFrameDecorator)decorator).yAxisLabel = yLabelText;
    ...
    public void render(Graphics2D g) {
        ...
        if (spp < 50.0) {
            ...
            while (wave.hasNext()) {
                st += step;
                y = wave.next();
                if (y == Wave.NO_DATA) gp.moveTo((float)getXPixel(st), lastY);
                else {
        ...
        if (axis != null) axis.postRender(g);
